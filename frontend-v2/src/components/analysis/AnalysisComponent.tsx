...
import { useAnalysisQueries } from '../../hooks/analysis';
...
export const AnalysisComponent = ({
  ...
  const { data, loading, error, refetch } = useAnalysisQueries();
  ...
  const renderTable = () => {
    ...
    if (items.length === 0) {
      return <p>No analysis records found</p>;
    }
  ...
  return (
    <div className={`analysis-component ${className || ''}`} style={{ padding: '16px' }}>
