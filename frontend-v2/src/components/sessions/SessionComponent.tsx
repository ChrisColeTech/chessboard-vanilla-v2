...
import { useSessionsQueries } from '../../hooks/sessions';
...
export const SessionComponent: React.FC<SessionComponentProps> = ({
  ...
  const { data, loading, error, refetch } = useSessionsQueries();
  ...
  const renderTable = () => {
    ...
    if (items.length === 0) {
      return <p>No session records found</p>;
    }
  ...
  return (
    <div className={`session-component ${className || ''}`} style={{ padding: '16px' }}>
