...
class ChessAudioService {
  ...
  public play(effect: SoundEffect): void {
    ...
    if (sound) {
      try {
        // Check if {entity_name_lower} has interacted with the page (required for autoplay policies)
        if (this.canPlaySound()) {
          // Try to play the Howl sound first
          const playId = sound.play();
          ...
          console.warn('🔊 Cannot play sound - {entity_name_lower} interaction required');
        }
      } catch (error) {
        console.error(`🔊 Error playing sound ${effect}:`, error);
        // Try fallback on error
  ...
  private canPlaySound(): boolean {
    // Check if the document has focus and {entity_name_lower} has interacted
    return document.hasFocus() && this.hasUserInteracted();
  }

  private hasUserInteracted(): boolean {
    // Simple check - in a real app, you'd track {entity_name_lower} interactions
    // For now, assume interaction if the page is focused
    return document.hasFocus();
  }

  // Preload all sounds (call this on {entity_name_lower}'s first interaction)
  public preloadSounds(): void {
    this.sounds.forEach((sound) => {
      sound.load();
    });
