...
export function useUIClickSoundOptimized() {
  ...
  const playUIClick = useCallback((context?: string) => {
    ...
    try {
      ...
      if (context) {
        console.log(`🔊 [UI CLICK OPTIMIZED] Playing click sound for: ${context}`);
      }
      
      return true;
    } catch (error) {
      console.warn(`🔊 [UI CLICK OPTIMIZED] Failed to play click sound:`, error);
      return false;
    }
  }, [playMove]);
