...
import { useProfilesQueries } from '../../../hooks/profiles';
...
export const ProfileComponent: React.FC<ProfileComponentProps> = ({
  ...
  const { data, loading, error, refetch } = useProfilesQueries();
  ...
  const renderTable = () => {
    ...
    if (items.length === 0) {
      return <p>No profile records found</p>;
    }
  ...
  return (
    <div className={`profile-component ${className || ''}`} style={{ padding: '16px' }}>
