...
import { useAnalyticsQueries } from '../../../hooks/analytics';
...
export const AnalyticsComponent: React.FC<AnalyticsComponentProps> = ({
  ...
  const { data, loading, error, refetch } = useAnalyticsQueries();
  ...
  const renderTable = () => {
    ...
    if (items.length === 0) {
      return <p>No analytics records found</p>;
    }
  ...
  return (
    <div className={`analytics-component ${className || ''}`} style={{ padding: '16px' }}>
