...
import { useStatsQueries } from '../../../hooks/stats';
...
export const StatsComponent: React.FC<StatsComponentProps> = ({
  ...
  const { data, loading, error, refetch } = useStatsQueries();
  ...
  const renderTable = () => {
    ...
    if (items.length === 0) {
      return <p>No stats records found</p>;
    }
  ...
  return (
    <div className={`stats-component ${className || ''}`} style={{ padding: '16px' }}>
