...
import { useSubscriptionsQueries } from '../../../hooks/subscriptions';
...
export const SubscriptionComponent: React.FC<SubscriptionComponentProps> = ({
  ...
  const { data, loading, error, refetch } = useSubscriptionsQueries();
  ...
  const renderTable = () => {
    ...
    if (items.length === 0) {
      return <p>No subscription records found</p>;
    }
  ...
  return (
    <div className={`subscription-component ${className || ''}`} style={{ padding: '16px' }}>
