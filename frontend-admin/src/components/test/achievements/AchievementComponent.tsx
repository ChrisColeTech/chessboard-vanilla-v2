...
import { useAchievementsQueries } from '../../../hooks/achievements';

interface AchievementComponentProps {
  className?: string;
}

export const AchievementComponent: React.FC<AchievementComponentProps> = ({
  className
}) => {
  const { data, loading, error, refetch } = useAchievementsQueries();
  ...
  return (
    <div className={`achievement-component ${className || ''}`} style={{ padding: '16px' }}>
