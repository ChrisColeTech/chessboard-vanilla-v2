...
import { useUsersQueries } from '../../../hooks/users';
...
export const UserComponent: React.FC<UserComponentProps> = ({
  ...
  const { data, loading, error, refetch } = useUsersQueries();
  ...
  const renderTable = () => {
    ...
    if (items.length === 0) {
      return <p>No user records found</p>;
    }
  ...
  return (
    <div className={`user-component ${className || ''}`} style={{ padding: '16px' }}>
