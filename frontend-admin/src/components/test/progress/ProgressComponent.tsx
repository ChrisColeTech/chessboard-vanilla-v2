...
import { useProgressQueries } from '../../../hooks/progress';
...
export const ProgressComponent: React.FC<ProgressComponentProps> = ({
  ...
  const { data, loading, error, refetch } = useProgressQueries();
  ...
  const renderTable = () => {
    ...
    if (items.length === 0) {
      return <p>No progress records found</p>;
    }
  ...
  return (
    <div className={`progress-component ${className || ''}`} style={{ padding: '16px' }}>
