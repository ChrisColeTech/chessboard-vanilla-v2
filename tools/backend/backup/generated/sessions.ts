...
// GET /validate/:token
router.get('/validate/:token', authenticate, async (req: any, res) => {
  try {
    const result = await sessionService.validateSession(req.body);
    res.json({ success: true, data: result });
  } catch (error: any) {
    res.status(400).json({ success: false, error: error.message });
  }
...
// DELETE /:token
router.delete('/:token', authenticate, async (req: any, res) => {
  try {
    const result = await sessionService.expireSession(req.body);
    res.json({ success: true, data: result });
  } catch (error: any) {
    res.status(400).json({ success: false, error: error.message });
  }
...
router.put('/:id', authenticate, async (req: any, res) => {
  try {
    const result = await sessionService.updateSession(req.params.id);
    res.json({ success: true, data: result });
  } catch (error: any) {
    res.status(400).json({ success: false, error: error.message });
  }
