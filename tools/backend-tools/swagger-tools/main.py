...
def update_backend_swagger_config(swagger_doc: dict, backend_path: str):
    ...
    if not os.path.exists(app_ts_path):
        print(f"❌ Backend app.ts not found: {app_ts_path}")
        return False
    ...
    try:
        ...
        if start_idx == -1 or end_idx == -1:
            print("❌ Could not find swagger configuration in app.ts")
            return False
  ...
  apis: ['./src/routes/*.ts', './src/app.ts'], // paths to files containing OpenAPI definitions
        ...
        print(f"✅ Updated swagger configuration in {app_ts_path}")
        return True
...
def main():
    ...
    parser.add_argument('--routes-dir', default='/mnt/c/Projects/chessboard-vanilla-v2/backend-v2/src/routes',
                       help='Directory containing TypeScript route files')
    parser.add_argument('--backend-path', default='/mnt/c/Projects/chessboard-vanilla-v2/backend-v2',
                       help='Path to backend directory')
    parser.add_argument('--output-dir', default='/mnt/c/Projects/chessboard-vanilla-v2/tools/backend-tools/swagger-tools',
    ...
    print("🚀 Chessboard V2 Swagger Documentation Generator")
    print("=" * 60)
    ...
    try:
        ...
        # Parse routes
        print(f"📂 Parsing routes from: {args.routes_dir}")
        ...
        print(f"✅ Parsed {len(parsed_routes)} files with {total_routes} total routes")
        
        # Generate documentation
        print("📝 Generating Swagger documentation...")
        ...
        print(f"   📄 Swagger JSON: {swagger_json_path}")
        print(f"   📄 Route docs: {route_docs_path}")
        ...
        print(f"   🏷️  Tags: {len(swagger_doc.get('tags', []))}")
        print(f"   📋 Schemas: {len(swagger_doc['components']['schemas'])}")
        print(f"   📁 Files processed: {len(parsed_routes)}")
