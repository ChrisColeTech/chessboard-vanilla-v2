...
export function useAppInitialization(): AppInitializationState & AppInitializationActions {
  ...
  // Removed selectedPieceSet - not needed for mini version
  const audioEnabled = useAppStore((state: any) => state.audioEnabled)
  const currentTheme = useAppStore((state: any) => state.currentTheme)
  const lastVisited = useAppStore((state: any) => state.lastVisited)
  ...
  }, [isInitializing, isInitialized, audioEnabled, currentTheme, lastVisited, handleProgress])
   ...
   */
  const prevPrefsRef = useRef({ audioEnabled, currentTheme })
  
  useEffect(() => {
    const prevPrefs = prevPrefsRef.current
    const currentPrefs = { audioEnabled, currentTheme }
    
    // Check if critical preferences changed after initialization
    if (isInitialized && (
      prevPrefs.audioEnabled !== audioEnabled ||
  ...
  }, [audioEnabled, currentTheme, isInitialized, startInitialization])
