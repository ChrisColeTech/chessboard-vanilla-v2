        ...
        initialize: async () => {
          try {
            ...
            if (session.isAuthenticated && session.user && session.token) {
              // Check if this is a demo token (bypass API verification)
              const isDemoToken = session.token === 'demo-token-dev-bypass' || 
                                 session.token === 'demo-token-ngrok-bypass';
              
              if (isDemoToken) {
                // Demo token - skip API verification and use stored data
                set({
                  user: session.user,
                  token: session.token,
                  progress: null, // session doesn't include progress, will be fetched separately
                  isAuthenticated: true,
                  isLoading: false,
                  error: null
                }, false, 'auth/initialize/demo-success');
                console.log('🚀 Demo session restored from storage');
                return;
              }
