...
import { useSelectedTab, useAppStore, type AppStore } from "./stores/appStore";
import { useChessAudio } from "./services/audio/audioService";
import { useAuthStatus } from "./hooks";
import AuthRouter from "./components/auth/AuthRouter";
import { WorkerTestPage } from "./pages/chess/WorkerTestPage";
...
function AppContent() {
  ...
  const setSelectedTab = useAppStore((state: AppStore) => state.setSelectedTab);
  const coinBalance = useAppStore((state: AppStore) => state.coinBalance);
  ...
  return (
    ...
    >
      ...
      {selectedTab === "uitests" && <div>UI Tests Page Coming Soon</div>}
      {selectedTab === "casino" && <div>Casino Page Coming Soon</div>}
