...
class ParentPageGenerator:
    ...
    def _create_parent_main_page(self, context: GenerationContext, template_name: str, variables: Dict[str, str]) -> None:
        ...
        file_path = context.pages_dir / config.page_id / f"{config.base_name}MainPage.tsx"
        self.file_writer.write_file(
            file_path,
            content,
            "ParentPageGenerator",
    ...
    def _create_parent_actions_hook(self, context: GenerationContext, template_name: str, variables: Dict[str, str]) -> None:
        ...
        file_path = context.hooks_dir / config.page_id / f"use{config.base_name}Actions.ts"
        self.file_writer.write_file(
            file_path,
            content,
            "ParentPageGenerator",
    ...
    def _create_mobile_parent_page(self, context: GenerationContext, variables: Dict[str, str]) -> None:
        ...
        file_path = context.pages_dir / config.page_id / f"Mobile{config.base_name}MainPage.tsx"
        self.file_writer.write_file(
            file_path,
            content,
            "ParentPageGenerator",
    ...
    def validate_parent_creation(self, context: GenerationContext) -> Dict[str, bool]:
        ...
        parent_main_path = context.pages_dir / config.page_id / f"{config.base_name}MainPage.tsx"
        validation_results['parent_main'] = parent_main_path.exists()
        
        # Check hook file
        hook_path = context.hooks_dir / config.page_id / f"use{config.base_name}Actions.ts"
    ...
    def get_created_files(self, context: GenerationContext) -> list:
        ...
        return [
            ...
            context.pages_dir / config.page_id / f"{config.base_name}MainPage.tsx",
            context.hooks_dir / config.page_id / f"use{config.base_name}Actions.ts",
    ...
    def update_parent_actions_config(self, context: GenerationContext) -> None:
        """Update parent actions configuration when children are added.""" 
        config = context.config
        
        print(f"🔄 Regenerating parent actions config for '{config.page_name}'")
        
        # Generate parent variables (includes child navigation actions for existing children)
        variables = self.variable_generator.generate_parent_variables(context)
        context.variables = variables
        
        # Regenerate parent actions configuration
        templates = self.template_engine.get_parent_templates()
        self._create_parent_actions_config(context, templates['parent_actions'], variables)
        
        print(f"✅ Parent actions config updated successfully!")
