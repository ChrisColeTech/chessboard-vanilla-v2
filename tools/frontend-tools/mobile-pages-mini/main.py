...
A streamlined version of the page generator that creates React pages using 
only dynamic templates, avoiding massive static file copying while maintaining
...
from modules.pre_generation_validator import PreGenerationValidator, ValidationError
...
class MobilePagesMiniCLI:
    """Main CLI interface for the mobile pages mini generator."""
    ...
    def __init__(self, frontend_root: str, force: bool = False, log_mode: str = "minimal", log_file: Optional[Path] = None):
        ...
        # Initialize logger
        self.logger = create_logger("mobile_pages_mini", log_mode, log_file)
        ...
        self.pre_validator = PreGenerationValidator(
            self.template_engine.static_dir,
            self.template_engine.dynamic_dir
        )
    ...
    def create_parent_page(self, name: str, mobile: bool = True) -> None:
        ...
        try:
            ...
            # PRE-GENERATION VALIDATION - Validate ALL dependencies before any file generation
            self.logger.operation("Running pre-generation dependency validation...")
            try:
                validation_results = self.pre_validator.validate_before_generation(context, "parent")
                if not validation_results.can_proceed:
                    self.logger.error("❌ Pre-generation validation failed!")
                    self.logger.error("Fix the above issues before proceeding.")
                    raise ValidationError("Critical dependencies missing - cannot generate parent page")
                self.logger.success("✅ Pre-generation validation passed - safe to proceed")
            except ValidationError as e:
                self.logger.error(f"Validation failed: {e}")
                sys.exit(1)
            
            # Perform template dependency analysis using scanner
            self.logger.success("Performing template dependency analysis using scanner")
            ...
            # Ensure only essential dependencies in mini version
            self.dependency_manager.ensure_all_dependencies_with_integration(context, None)
            ...
            # Generate dynamic navigation files after page is registered
            self.logger.info("Generating dynamic navigation files...")
            self.dependency_manager._create_navigation_files_dynamically(context)
            
            # Validate creation
            validation = self.parent_generator.validate_parent_creation(context)
            self._print_validation_results("Parent Page", validation)
            
            # Navigation files are generated dynamically (no static components in mini version)
            self.logger.success("Parent page creation completed with dynamic navigation generation")
    ...
    def create_child_page(self, name: str, parent: str, mobile: bool = True) -> None:
        ...
        try:
            ...
            # Create page configuration
            config = PageConfig(name=name, parent=parent, mobile=mobile, description="")
            ...
            self.logger.operation("Running pre-generation dependency validation...")
            try:
                validation_results = self.pre_validator.validate_before_generation(context, "child")
                if not validation_results.can_proceed:
                    self.logger.error("❌ Pre-generation validation failed!")
                    self.logger.error("Fix the above issues before proceeding.")
                    raise ValidationError("Critical dependencies missing - cannot generate child page")
                self.logger.success("✅ Pre-generation validation passed - safe to proceed")
            except ValidationError as e:
                self.logger.error(f"Validation failed: {e}")
                sys.exit(1)
            ...
            # Navigation files are generated dynamically (no static components in mini version)
            self.logger.success("Child page creation completed with dynamic navigation generation")
    ...
    def _update_parent_for_new_child(self, context: GenerationContext, child_name: str) -> None:
        ...
        for child_id in existing_children:
            # Read the actual page info from pages.config.json to preserve original naming
            page_info = self.config_integration.get_page_info(child_id)
            if page_info:
                # Use the original name from config to preserve capitalization
                child_config = PageConfig(
                    name=page_info['name'], 
                    parent=config.parent,
                    mobile=page_info.get('has_mobile', False),
                    description=page_info.get('description', '')
                )
                child_configs.append(child_config)
            else:
                # Fallback to ID-based naming if config not found (shouldn't happen)
                child_name_proper = NameStandardizer.to_pascal_case(child_id)
                child_config = PageConfig(name=child_name_proper, parent=config.parent, mobile=True, description="")
                child_configs.append(child_config)
        
        # Create parent context for updating
        parent_config = PageConfig(name=config.parent, parent=None, mobile=True, description="")
        ...
        # Update parent actions config (this was missing!)
        self.parent_generator.update_parent_actions_config(parent_context)
    ...
    def create_pages(self, parent: str, children: list = None, mobile: bool = True) -> None:
        ...
        try:
            ...
            self.create_parent_page(parent, mobile)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Mobile Pages Mini - Template-Based Page Generator",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s parent Settings
  %(prog)s child UserProfile --parent Settings
  %(prog)s create Dashboard --children Analytics Reports
  %(prog)s analyze
  %(prog)s validate
        """
    )
    ...
    try:
        ...
        cli = MobilePagesMiniCLI(frontend_root, args.force, log_mode, args.log_file)
        ...
        if args.command == 'parent':
            cli.create_parent_page(args.name, True)  # Always mobile
        elif args.command == 'child':
            cli.create_child_page(args.name, args.parent, True)  # Always mobile
        elif args.command == 'create':
            cli.create_pages(args.parent, args.children, True)  # Always mobile
