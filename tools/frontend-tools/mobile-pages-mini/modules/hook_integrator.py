...
class HookIntegrator:
        ...
        if page_type == 'parent':
            ...
            if not hook_path.exists():
                ...
                for child in children:
                    child_name = child.get('name', NameStandardizer.to_pascal_case(child['id']))
    ...
    def _get_parent_hook_path(self, parent_id: str) -> Path:
        ...
        parent_name = NameStandardizer.to_pascal_case(parent_id)
        return self.hooks_dir / f"{parent_id}/use{parent_name}Actions.ts"
    
    def _get_child_hook_path(self, child_id: str, parent_id: str) -> Path:
        """Get path to child action hook."""
        # Get the child name from config instead of converting ID
        child_info = self._get_page_info(child_id)
        child_name = child_info.get('name', NameStandardizer.to_pascal_case(child_id)) if child_info else NameStandardizer.to_pascal_case(child_id)
        # Remove 'Page' suffix if present for hook naming - use NameStandardizer for consistency
        child_name = NameStandardizer.to_pascal_case(child_name.replace('Page', '')) if child_name.endswith('Page') else NameStandardizer.to_pascal_case(child_name)
        return self.hooks_dir / f"{parent_id}/use{child_name}Actions.ts"
    ...
    def _generate_parent_hook_content(self, parent_id: str, parent_info: Dict, children: List[Dict]) -> str:
        ...
        parent_name = parent_info.get('name', NameStandardizer.to_pascal_case(parent_id))
        ...
        for child in children:
            # Use base name without "Page" suffix for navigation methods - use NameStandardizer for consistency
            raw_child_name = child.get('name', NameStandardizer.to_pascal_case(child['id']))
            child_name = NameStandardizer.to_pascal_case(raw_child_name.replace('Page', '')) if raw_child_name.endswith('Page') else NameStandardizer.to_pascal_case(raw_child_name)
            method_name = f"goTo{child_name}"
...
{chr(10).join(navigation_methods) if navigation_methods else '  // No navigation methods - no children found'}
  ...
  return {};
    ...
    def _generate_child_hook_content(self, child_id: str, child_info: Dict, siblings: List[Dict]) -> str:
        ...
        # Use base name without "Page" suffix for hook naming - use NameStandardizer for consistency
        raw_name = child_info.get('name', NameStandardizer.to_pascal_case(child_id))
        child_name = NameStandardizer.to_pascal_case(raw_name.replace('Page', '')) if raw_name.endswith('Page') else NameStandardizer.to_pascal_case(raw_name)
        ...
        for sibling in siblings:
            # Use base name without "Page" suffix for sibling navigation - use NameStandardizer for consistency
            raw_sibling_name = sibling.get('name', NameStandardizer.to_pascal_case(sibling['id']))
            sibling_name = NameStandardizer.to_pascal_case(raw_sibling_name.replace('Page', '')) if raw_sibling_name.endswith('Page') else NameStandardizer.to_pascal_case(raw_sibling_name)
