...
const analyzePosition = async (fen: string) => {
  try {
    ...
    const cachedResponse = await fetch(
      `/api/analysis/stored/${encodeURIComponent(fen)}`,
      {
        headers: { Authorization: `Bearer ${getToken()}` },
      }
    );
...
const submitPuzzleSolution = async (moves: string[]) => {
  try {
    ...
    const response = await fetch(
      `/api/puzzles/${puzzleState.currentPuzzle.id}/solve`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
const getPuzzleHint = async () => {
  try {
    ...
    const response = await fetch(
      `/api/puzzles/${puzzleState.currentPuzzle.id}/hint`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
const getCachedData = (key: string, maxAge = 300000): any => {
  // 5 minutes default
